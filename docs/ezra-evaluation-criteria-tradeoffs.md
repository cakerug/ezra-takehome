...
- **Rewriting every row's `Order` to a dense `0..N-1` run sidesteps position-calculation fragility entirely.** The alternative — computing a position for just the moved row (e.g. splitting the gap between its new neighbors) — is a well-known source of bugs: gap/fractional keys eventually run out of room between two adjacent values and need periodic renormalization. A full rewrite never accumulates that debt; every reorder leaves a clean, dense sequence, at the cost of an O(N) write per move instead of O(1). See the comment on `TaskOperations.ReorderAsync` for the same reasoning stated at the code site.

**Trade-off:** the payload grows with the number of projects, and — more fundamentally than payload size — **this design is incompatible with pagination**, not just slower under it. See Pagination under At scale below for why, and the fractional-indexing alternative that would replace it if that ever becomes necessary.
...
- **Routing.** There's no router today — `App.tsx` is the whole app; `selectedProjectId` (`App.tsx:89`) is plain `useState`, prop-drilled into `ProjectSidebar` and `ContentArea`, with no URL reflecting the current selection (no deep-linking, no back/forward, no shareable `/projects/{id}` link). Multiple views/pages (surface area) would need react-router (or similar) wrapping `<App />` in `main.tsx`, swapping `selectedProjectId` state for `useParams()`/`navigate()`, and turning the existing stale-selection fallback logic (`App.tsx:101-105`) into a redirect. No component restructuring needed — `ContentArea`/`ProjectSidebar` keep their current props either way. Scoped this out as a half-day, not a large change, when asked directly; not pursued since it's URL polish rather than a functional gap for a single-user local app.

  **If a slug were added later** (`/projects/{id}-{slug}` rather than a bare id): generate and persist a `Slug` column on `Project` at create/rename time, not derive it on the fly from `Name` at read time. Two reasons — uniqueness (two projects can share a name; deduping requires a DB lookup regardless, so persisting the result avoids doing that lookup on every read) and stability (a read-time-derived slug changes silently whenever the project is renamed, breaking any bookmarked/shared link). Routing on `{id}-{slug}` rather than the slug alone keeps the id as the real lookup key and the slug purely decorative, so a rename never breaks the link and the slug column doesn't even need a uniqueness constraint. Not pursued for the same reason as routing above — no shareable/public URLs exist yet for a slug to serve.
...
- **Auth/authorization.** The biggest gap — there's currently none. Any real multi-user scaling (users) requires this before anything else, and it reshapes `Operations/` (ownership checks) and `Endpoints/` (auth middleware/policies). Everything else in this list assumes single-tenant until this lands.
  ...
  **What it would give us: multi-tenancy, not security.** Worth being precise about this rather than reaching for auth reflexively — there's no sensitive data here (it's a todo list), and CORS is already dev-locked to loopback origins with no credentials involved, so there's no real adversarial threat model this app faces today. The entire payoff of adding auth would be letting more than one person use the app without seeing each other's projects/tasks. If that's not a goal, adding auth is scope creep dressed up as rigor, not a gap.

  **The design, if/when multi-user becomes a real requirement:** JWT bearer tokens (`AddAuthentication().AddJwtBearer(...)` in `Program.cs`) over cookie-based auth, since there's no cross-origin credential/CSRF surface to manage that way. A `User` entity (`Id`, `Email`, `PasswordHash` via `PasswordHasher<User>` rather than pulling in full ASP.NET Identity) plus a `UserId` FK on `Project` only — `TaskItem` inherits ownership through its project, no need to duplicate the column. Every existing query in `Operations/` gets a `UserId` filter (e.g. `EnsureProjectExistsAsync` picks up a `userId` parameter); the route groups in `TaskEndpoints`/`ProjectEndpoints` get `.RequireAuthorization()`. The riskiest part is exactly that filter — forgetting it on one query path is a cross-user data leak, not a crash, so it wouldn't just be implemented but specifically tested: a 404 (not the data) when a resource belongs to another user, per endpoint.

  Not implemented — no multi-user requirement exists for this app today — but recorded so the design (and the "multi-tenancy, not security" framing) doesn't need re-deriving if that requirement shows up.
- **Pagination.** Endpoints return full collections with no `Skip`/`Take`. Needed as soon as task/project counts grow (users/data volume) — also unblocks the frontend virtualization item above.
  ...
  **This is a real design conflict, not just a missing feature.** Both reorder endpoints (`ProjectOperations.ReorderAsync`/`TaskOperations.ReorderAsync` — see Project reordering above) rewrite the *entire* list's `Order` to a dense `0..N-1` run on every drag-drop, and validate that the submitted id list is exactly the full current set (`SetEquals`). A paginated client physically can't satisfy that: it only holds one page's worth of ids, so it could never submit a request that passes validation, and there'd be no drop target for the off-page remainder of the list anyway. Pagination and the current reorder design are mutually exclusive as built — this can't be added incrementally without changing how `Order` works.

  **The fix, if/when task or project counts justify pagination: fractional/rank-based positioning.** Give each row its own independently-sortable position value between its neighbors, so a move becomes a single-row `UPDATE` to a value between the new neighbors instead of a rewrite of the whole list. Listing stays a plain `ORDER BY position`, which pages/keyset-paginates for free — a move no longer needs to know or touch anything about rows outside the current page. Two variants exist, distinguished by what the position value actually is:
    - **Numeric (float) ranks** — Trello's `pos` field. Simple, but a fixed-precision `double` has a hard ceiling: enough inserts at the same spot and two adjacent values collide, forcing a rebalance (Trello renumbers a local cluster once adjacent floats land within ~0.0001 of each other).
    - **String/lexicographic ranks** — an "alphabet" of characters instead of digits, so a new rank is generated by finding a string that sorts between two others (base62 in Jira's LexoRank; base-95, using the full ASCII range, in Figma's layer-ordering scheme — both documented in their own engineering posts; a couple of secondary sources also attribute this approach to Linear, though that one isn't confirmed against a primary source). Same single-row-update property as the float version, but a string can be subdivided far more times before two adjacent ranks collide — you're growing the string by a character instead of running out of floating-point mantissa bits — so the rebalance trigger is pushed out much further, at the cost of ranks slowly growing longer over time.

  **Preferred here: string/lexicographic ranks over floats**, precisely for that rebalance-frequency reason — it's the same underlying idea as floats, just with a much larger, cheaper-to-extend keyspace before maintenance work is ever needed.

  Not implemented — no list here is remotely close to needing pagination — but recorded so the choice doesn't need re-deriving when it does.
- **Search/filtering.** No endpoint supports filtering by title text or completion status — clients would have to fetch the full collection and filter client-side. Worth adding as a query param on the existing list endpoints (`?q=`, `?status=complete`) once task counts grow enough that scanning the full list client-side is wasteful (users/data volume). An indexed `LIKE`/EF Core `.Contains()`, or SQLite FTS5 if ranking/tokenized matching is ever needed, is the right-sized tool here — a dedicated search service (Elasticsearch/OpenSearch) solves relevance ranking and fuzzy matching at millions-of-documents scale, which a single user's task list never approaches; reaching for one here would be infra cost with no matching problem.
...
- **Optimistic concurrency on edits.** `TaskOperations.UpdateAsync`/`ProjectOperations.UpdateAsync` are last-write-wins today — two concurrent edits to the same row, and whichever request saves second silently overwrites the first with no signal to either client. This is a different race from the FK-constraint one `MoveAsync` already closes (see Error handling strategy above): that one is *cross-row* (a task vs. its target project) and already caught for free by SQLite's FK enforcement plus the `DbUpdateException` → 409 middleware mapping; a same-row field conflict has no FK to violate, so it needs its own mechanism. Two ways to add it, both requiring a `Version` column marked `IsConcurrencyToken()` and a catch for `DbUpdateConcurrencyException` (a subtype of `DbUpdateException`, so it already reaches the middleware today — it would just need a distinct branch for the right status/message):
    - **App-level `Version` field**, round-tripped through the request/response DTOs — simpler, fits the existing DTO pattern.
    - **HTTP `ETag`/`If-Match` headers** — the REST-correct version (412 Precondition Failed rather than 409; keeps concurrency metadata out of the DTO), at the cost of hand-rolled header parsing/quoting and a new status-code branch in the middleware.

  Not needed today (single client, no concurrent editors), but worth having designed before a second concurrent editor — multi-device, or an auth-enabled multi-user mode — is in scope (users axis).
