...
- **Frontend-Backend Communication**:
    ...
    - **Query Framework**: I used react-query because our product/data schema is simple (as opposed to GraphQL/Apollo which would be well-suited for a more complex data schema or complex product usage of data). I also tried to keep the data flow simple - simply re-fetch each time for new data and err towards refetching because it's cheap and fast and is less bugprone compared to more imperative data handling. I didn't do any fancy optimistic writes etc for the same reason.
