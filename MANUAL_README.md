...
- **Frontend-Backend Communication**:
    ...
    - **Query Framework**: I used react-query because our product/data schema is simple (as opposed to GraphQL/Apollo which would be well-suited for a more complex data schema or complex product usage of data). Instead of configuring it with a longer staleTime or any optimistic writes, I erred towards more refetching because it's cheap, esp with react-query's caching and refetch in background and request deduping. React-query also gives retry, loading/error states for free.
